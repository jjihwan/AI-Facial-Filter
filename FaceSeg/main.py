...
def main(config):
    ...
    if config.train:
        ...
        '''transform = Compose([RandomSizedCrop(size=config.imsize), RandomAffine(degrees=30, translate=(0.2*config.imsize, 0.2*config.imsize), scale=(0.5, 1.25)), RandomHorizontallyFlip(p=.5), RandomSized(
            size=config.imsize), AdjustBrightness(bf=0.1), AdjustContrast(cf=0.1), AdjustHue(hue=0.1),
                             AdjustSaturation(saturation=0.1)])'''
        transform = Compose([RandomSizedCrop(size=config.imsize), RandomHorizontallyFlip(p=.5), RandomSized(
            size=config.imsize), AdjustBrightness(bf=0.1), AdjustContrast(cf=0.1), AdjustHue(hue=0.1),
                             AdjustSaturation(saturation=0.1)])
